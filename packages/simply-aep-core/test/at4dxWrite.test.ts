...
describe('updateBinding', () => {
  ...
  it("preserves an existing local file's shape (field order, indentation, comment) when only one field changes", async () => {
    ...
    const filePath = path.join(customMetadataDir, 'ApplicationFactory_SelectorBinding.Account_Selector.md-meta.xml');
...
`;
    ...
    expect(writtenXml).toBe(
      existingXml.replace('<value xsi:type="xsd:double">1</value>', '<value xsi:type="xsd:double">5</value>'),
    );
  });

  it('falls back to full-document generation for a file whose <value> element is in an unrecognized shape', async () => {
    const customMetadataDir = path.join(tmpDir, 'customMetadata');
    await fsp.mkdir(customMetadataDir, { recursive: true });
    const filePath = path.join(customMetadataDir, 'ApplicationFactory_SelectorBinding.Account_Selector.md-meta.xml');
    const existingXml = `<?xml version="1.0" encoding="UTF-8"?>
...
`;
    ...
    // Changes `to` specifically — the field whose <value> element is the unrecognized one, so this
    // update can only succeed by falling back to full-document generation.
    const result = await updateBinding(
      { bindingType: 'Selector', developerName: 'Account_Selector', to: 'NewAccountsSelector' },
      { sourceDirs: [tmpDir] },
    );

    expect(result.issues).toEqual([]);
    const { records } = scanLocalBindings([tmpDir], ['Selector']);
    expect(records[0]).toMatchObject({ key: 'Account', to: 'NewAccountsSelector', priority: 1 });
  });
});
